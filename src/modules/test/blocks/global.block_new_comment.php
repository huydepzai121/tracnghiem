...
if (!nv_function_exists('nv_test_comment_new')) {
    ...
    function nv_test_comment_new($block_config)
    {
        global $db, $site_mods, $db, $module_info, $global_config;
        ...
        if (!empty($array_news_id)) {
            $result = $db->query('SELECT t1.id, t1.alias AS alias_id, t2.alias AS alias_cat FROM ' . NV_PREFIXLANG . '_' . $mod_data . '_exams t1 INNER JOIN ' . NV_PREFIXLANG . '_' . $mod_data . '_cat t2 ON t1.catid = t2.id WHERE t1.id IN (' . implode(',', array_unique($array_news_id)) . ') AND t1.status=1 AND t1.isfull=1 AND t2.status=1');
            $array_news_id = array();
...
if (defined('NV_SYSTEM')) {
    $content = nv_test_comment_new($block_config);
}
