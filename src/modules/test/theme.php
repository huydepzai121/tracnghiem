...
{
    global $module_file, $nv_Lang, $module_info, $op;
...
{
    global $module_file, $nv_Lang, $module_info, $client_info, $module_config, $array_test_cat, $module_name;
...
{
    global $module_file, $nv_Lang, $module_info, $client_info, $array_test_cat, $module_config, $module_name;
...
{
    global $module_name, $module_file, $nv_Lang, $module_info, $array_config, $module_data, $db, $module_config;
...
{
    global $module_name, $module_file, $nv_Lang, $module_info, $op, $array_config, $client_info, $array_test_cat;
...
function nv_theme_test_detail2($exam_info, $array_data, $array_saved_data, $tester_answer, $base_url, $content_comment, $array_keyword, $array_other, $related_new_array, $related_array, $topic_array, $array_listcat_menu, $examinations_subject)
{
    ...
    $config_user_view = $private_config_site['config_user_view'];
    if (defined('NV_IS_USER')) {
        ...
        if (!empty($array_fields)) {
            foreach ($array_fields as $row) {
                ...
                if (!empty($config_user_view) &&  !in_array($row['field'], $config_user_view)) continue;
    ...
    if (!empty($array_saved_data) && !empty($array_saved_data['begintime'])) {
        ...
        if ( $array_config['enable_social']
            && ( !isset($private_config_site['detail_share_now']) ||  (!empty($private_config_site['detail_share_now'])))) {
            if ($exam_info['type'] == 0) {
    ...
    if (!empty($content_comment) && ( !isset($private_config_site['detail_comment_view']) ||  (!empty($private_config_site['detail_comment_view'])))) {
        $xtpl->assign('COMMENT', $content_comment);
        $xtpl->parse('main.comment');
    }
    ...
    if (empty($array_saved_data)) {
        ...
        if ($exam_info['isdone'] && $exam_info['exams_type'] > 0) {
            ...
            if ($exam_info['view_mark_after_test']) {
                $xtpl->assign('score_title', $exam_info['exams_type'] == 1 ? $nv_Lang->getModule('score') : $nv_Lang);
