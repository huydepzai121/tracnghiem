...
if ($nv_Request->isset_request('topicsid', 'post')) {
    ...
    foreach ($listid as $_id) {
        $db->query('UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_exams SET topicid=' . $topicsid . ' WHERE id=' . $_id);
        
        $result = $db->query('SELECT listcatid FROM ' . NV_PREFIXLANG . '_' . $module_data . '_exams WHERE id=' . $_id);
