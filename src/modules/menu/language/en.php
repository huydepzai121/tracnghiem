<?php

/**
 * NukeViet Content Management System
 * @version 4.x
 * @author VINADES.,JSC <contact@vinades.vn>
 * @copyright (C) 2009-2023 VINADES.,JSC. All rights reserved
 * @license GNU/GPL version 2 or any later version
 * @see https://github.com/nukeviet The NukeViet CMS GitHub project
 */

if (!defined('NV_MAINFILE')) {
    exit('Stop!!!');
}

$lang_translator['author'] = 'VINADES.,JSC <contact@vinades.vn>';
$lang_translator['createdate'] = '20/07/2023, 07:15';
$lang_translator['copyright'] = '@Copyright (C) 2010 VINADES.,JSC. All rights reserved';
$lang_translator['info'] = '';
$lang_translator['langtype'] = 'lang_module';

$lang_module['menu_manager'] = 'Menu management';
$lang_module['type_menu_manager'] = 'Management menu type';
$lang_module['add_menu'] = 'Add Menu';
$lang_module['edit_menu'] = 'Edit menu';
$lang_module['menu_name'] = 'Menu name';
$lang_module['menu_description'] = 'Description';
$lang_module['error_menu_name'] = 'Error: You do not give the name of the menu';
$lang_module['menu_number'] = 'Number of elements';
$lang_module['save'] = 'Save';
$lang_module['edit'] = 'Edit';
$lang_module['delete'] = 'Delete';
$lang_module['errorsave'] = 'Error: The system can not update data. The menu name may be dupplicated. Please try other name';
$lang_module['number'] = 'Order';
$lang_module['type_header'] = 'Top Menu';
$lang_module['type_along'] = 'Vertical Menu';
$lang_module['type_footer'] = 'Bottom menu';
$lang_module['menu'] = 'List menu';
$lang_module['type_tree'] = 'Menu tree folder';
$lang_module['m_list'] = 'Menu list';
$lang_module['add_type_menu'] = 'Select menu type';
$lang_module['module_name'] = 'Select module';
$lang_module['cho_module'] = 'Select module';
$lang_module['error_menu_block'] = 'Please enter menu block name';
$lang_module['action'] = 'Activities';
$lang_module['display'] = 'Display';
$lang_module['data_no'] = 'There is no data in the system';
$lang_module['back'] = 'Back';
$lang_module['add_item'] = 'Add Item';
$lang_module['title'] = 'Item name';
$lang_module['item_menu'] = 'Elements of module';
$lang_module['chomodule'] = 'Link to Module';
$lang_module['cho_module_item'] = 'Link to Module Item';
$lang_module['select'] = 'Select';
$lang_module['note'] = 'Notes';
$lang_module['link'] = 'Link';
$lang_module['module'] = 'Module';
$lang_module['op'] = 'op';
$lang_module['path'] = 'path';
$lang_module['target'] = 'Open link';
$lang_module['type_target1'] = 'Current Page';
$lang_module['type_target2'] = 'Open new tab';
$lang_module['type_target3'] = 'Open new window';
$lang_module['cat'] = 'This menu has';
$lang_module['cat0'] = 'Main item';
$lang_module['cho_item'] = 'Choose the type of module';
$lang_module['cats'] = 'Belong to Item';
$lang_module['caton'] = 'submenu, if you delete the submenu will be removed?';
$lang_module['name_block'] = 'Menu block';
$lang_module['here'] = 'here';
$lang_module['groups'] = 'Select groups';
$lang_module['sub_menu'] = 'Sub menu';
$lang_module['main_note'] = 'Click the name of each menu block below to add and edit the menu items. Hint: You can create multiple menu block to use each the theme and positions.';
$lang_module['add_type_active'] = 'Add an active effect to the menu when the current page URL';
$lang_module['add_type_active_0'] = 'matches the menu link';
$lang_module['add_type_active_1'] = 'has the same beginning as the menu link';
$lang_module['add_type_active_2'] = 'contains a menu link';
$lang_module['add_type_css'] = 'Assign the css';
$lang_module['add_error_module'] = 'Error: No any module selected';
$lang_module['add_error_module_exist'] = 'Error: Module not exist';
$lang_module['action_menu_reload'] = 'Menu reload';
$lang_module['action_menu_reload_confirm'] = 'This action will reload the ingredients is the subject of the current module. Do you want to continue?';
$lang_module['action_menu_reload_success'] = 'Already successfully reloaded menu!';
$lang_module['action_menu_reload_none_success'] = 'Load unsuccessful menu. Please check the operation.';
$lang_module['action_menu_reload_note'] = 'Reload the children was the subject of module';
$lang_module['action_menu'] = 'Import form';
$lang_module['action_menu_sys_1'] = 'Module name';
$lang_module['action_menu_sys_2'] = 'Module name, submenu is catalogs or functions';
$lang_module['action_menu_sys_3'] = 'Catalogs or functions in module';
$lang_module['icon'] = 'Icon';
$lang_module['image'] = 'Image';
$lang_module['action_form'] = 'Submit';
$lang_module['msgnocheck'] = 'No menu selected';
$lang_module['id'] = 'ID';
$lang_module['click_to_view'] = 'Click to view';
$lang_module['groups_view'] = 'Right to view';
$lang_module['no_submenu'] = 'There is no submenu';
$lang_module['no'] = 'No';
$lang_module['multi_delete'] = 'Delete selected menus';
$lang_module['select_menu_block'] = 'Select menu block';
$lang_module['module_display'] = 'Modules are displayed';
$lang_module['menu_note_auto'] = 'Note: This block gets the menu automatically';
$lang_module['title_length'] = 'Title length';
