...
$widget_info = [
    ...
    'func' => function () {
        ...
        $_arr_siteinfo = [];
        $cacheFile = 'widget_cmttotal_' . NV_CACHE_PREFIX . '.cache';
        $cacheTTL = 1800;

        if (($cache = $nv_Cache->getItem($module_name, $cacheFile, $cacheTTL)) != false) {
            $_arr_siteinfo = unserialize($cache);
        } else {
            $_arr_siteinfo['total_comments'] = $db->query('SELECT COUNT(*) FROM ' . NV_PREFIXLANG . '_' . $module_data)->fetchColumn();

            $nv_Cache->setItem($module_name, $cacheFile, serialize($_arr_siteinfo), $cacheTTL);
        }

        $tpl->assign('NUM', nv_number_format($_arr_siteinfo['total_comments']));

        return $tpl->fetch('widget_cmttotal.tpl');
    }
];
