...
foreach ($hook_sys as $tag => $data) {
    echo '  * [[<a style="text-decoration: none;" href="?f=' . urlencode($data['file']) . '&amp;c=' . urlencode($data['code']) . '#highlight" target="_blank">nukeviet4:codex:hooks-reference:' . $tag . '|' . $tag . "</a>]]\n";
}

foreach ($hook_modules as $module => $datas) {
    echo "\n<h2 style=\"margin:0;padding:0;\">===== Hook của module " . $module . " =====</h2>\n";
    foreach ($datas as $tag => $data) {
        echo '  * [[<a style="text-decoration: none;" href="?f=' . urlencode($data['file']) . '&amp;c=' . urlencode($data['code']) . '#highlight" target="_blank">nukeviet4:codex:hooks-reference:' . $module . ':' . $tag . '|' . $tag . "</a>]]\n";
    }
}
